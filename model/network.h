...
#include "ross.h"
#include "util/parse_new_config.h"
...
typedef struct {
    ...
    tw_lpid src; // Source switch LP ID
    tw_lpid dest; // Final destination switch LP ID
    char destIP[16]; // Final destination switch IP
    tw_lpid prev_hop; // LP ID of the previous hop switch
    tw_lpid next_hop; // LP ID of the next hop switch
    int size_in_bytes;
...
typedef struct {
    ...
    config *conf;
    char *dest_ip; // the destination IP of the packet generated by this switch
    double traffic_gen_load; // the load of the generated traffic, percentage of the output port bandwidth
    stats *stats;
} switch_state;
...
extern void init_switch_to_lp(char *filename);
extern tw_lpid *switch_to_lp; // the mapping from switch ID to LP ID
extern tw_lpid switch_id_to_lp_id(tw_lpid switch_id);
extern tw_lpid lp_id_to_switch_id(tw_lpid lpid);
